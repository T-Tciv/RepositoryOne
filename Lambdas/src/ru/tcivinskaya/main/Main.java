...
public class Main {
    public static void main(String[] args) {
        ...
        List<String> namesWithoutRepetitions = people.stream().map(Person::getName).distinct().collect(Collectors.toList());

        System.out.println("Вывод списка уникальных имён:");
        System.out.println(namesWithoutRepetitions.stream().collect(Collectors.joining(", ", "Имена: ", ".")));
        System.out.println();

        //Получение списка людей младше 18
        List<Person> youngPeople = people.stream().filter(x -> x.getAge() < 18).collect(Collectors.toList());

        System.out.println("Получение среднего возраста людей младше 18:");
        System.out.println(youngPeople.stream().mapToInt(Person::getAge).sum() / (double) youngPeople.size());
        System.out.println();

        System.out.println("Получение Map, в котором ключи - имена, значения - средний возраст:");
        System.out.println(people.stream().collect(Collectors.groupingBy(Person::getName, Collectors.averagingInt(Person::getAge))));
        System.out.println();

        System.out.println("Вывод имён людей, чей возраст варьируется от 20 до 45, в порядке убывания возраста:");
        people.stream().filter(p -> p.getAge() <= 45 && p.getAge() >= 20).sorted((p1, p2) -> p2.getAge() - p1.getAge()).forEach(p -> System.out.println(p.getName()));
    }
}
